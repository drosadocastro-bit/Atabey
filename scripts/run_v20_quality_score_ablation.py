...
class TrainHybridEvalSummary:
    ...
    total_division_tp: int
    total_division_fp: int
    total_division_fn: int
    mean_division_jaccard: float | None
...
def _record_for_graph(
    *,
    ...
    return TrainHybridEvalRecord(
        ...
        division_tp=report.division_tp,
        division_fp=report.division_fp,
        division_fn=report.division_fn,
        division_jaccard=report.division_jaccard,
...
def _build_summaries(records: list[TrainHybridEvalRecord]) -> list[TrainHybridEvalSummary]:
    ...
    for route in routes:
        ...
        mean_recall = float(mean(recalls)) if recalls else None
        mean_edge_recall = float(mean(edge_recalls)) if edge_recalls else None
        mean_div_jaccard = float(mean(division_jaccards)) if division_jaccards else None

        scores = [r.quality_score for r in items]
        quality_total = float(sum(scores))
        ...
        summaries.append(
            TrainHybridEvalSummary(
                ...
                total_division_fp=sum(r.division_fp for r in items),
                total_division_fn=sum(r.division_fn for r in items),
                mean_division_jaccard=mean_div_jaccard,
