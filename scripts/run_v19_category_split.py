...
@dataclass
class CategorySplitReport:
    sample_id: str
    actionable_edges: int
    category_a_edges: int
    category_b_edges: int

def evaluate_split(graph, ground_truth):
    ...
    for source_id, target_id in newly_evaluable:
        ...
        if dist <= 9.0:
            actionable_count += 1
            # Check if this edge exists in the tracker's output
            if (s_pred, t_pred) in predicted_edges:
                cat_a_count += 1
            else:
                cat_b_count += 1
                
    return CategorySplitReport(
    ...
    for sample_id in sample_ids:
        ...
        if _should_use_cfar_route(profile=profile, adaptive_detector=settings.detector, cfar_route_policy=DEFAULT_HYBRID_FROZEN_DEFAULTS.cfar_route_policy):
            graph, _ = build_graph_cfar_sidelobe(
                ...
                max_timepoints=100, # Use 100 to perfectly match the V13 baseline conditions
            )
        else:
            graph, _, _, _, _, _ = _build_v9_style_graph(sample_path, max_timepoints=100)
            
        report = evaluate_split(graph, ground_truth)
        reports.append(asdict(report))
        
        with open("submissions/v19_category_split.json", "w") as f:
            json.dump(reports, f, indent=2)
            
    summary = {
        "total_actionable_edges": sum(r["actionable_edges"] for r in reports),
        "total_category_a_edges": sum(r["category_a_edges"] for r in reports),
        "total_category_b_edges": sum(r["category_b_edges"] for r in reports),
    }
    
    with open("submissions/v19_category_split_summary.json", "w") as f:
        json.dump(summary, f, indent=2)
        
    print("V19 Category Split complete.")
